...
export default {
  ...
  methods: {
    ...
    async processClient(resolvedClient) {
      ...
      if (resolvedClient.Reachable) {
        ...
        if (idx == -1) {
          ...
          this.clientInfosOffline[idx].Refreshing = false;
        }
      }
      if (this.init) {
    ...
    async fillClientInfoArrays(client) {
      try {
        ...
        if (idy == -1) {
          ...
          this.clientInfosOffline[idy].Refreshing = false;
        }
      }
    },
    /**
    ...
    async fetchClientData(client) {
      const clientInfo = await get(client.Address);
      clientInfo.Ip = client.Address;
      clientInfo.HostName = client.HostName;
      clientInfo.EncoderLineOut = await get(client.Address + "/encoder/");
    ...
    pingOffline: async function () {
      ...
      this.clientInfosOffline.forEach((client) => (client.Refreshing = true));
