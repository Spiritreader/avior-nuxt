...
export default {
  ...
  computed: {
    ...
    isLoading: function () {
      return this.client.Encoder && (this.client.Encoder.Active || this.client.Mover.Active || this.client.FileWalker.Active);
    },
  ...
  methods: {
      ...
      try {
        ...
        this.ws.onmessage = (info) => {
          if (!this.shutdownMachine) {
            ...
            this.client.HostName = this.clientInit.HostName;
          }
        };
        this.ws.onclose = () => {
    ...
    getMainLog: async function () {
      ...
      if (!this.showMainLog) {
        const log = await get(this.client.Ip + "/logs/main/");
    ...
    getErrorLog: async function () {
      ...
      if (!this.showErrorLog) {
        const log = await get(this.client.Ip + "/logs/err/");
    ...
    getProcessedLog: async function () {
      ...
      if (!this.showProcessedLog) {
        const log = await get(this.client.Ip + "/logs/processed/");
    ...
    getSkippedLog: async function () {
      ...
      if (!this.showSkippedLog) {
        const log = await get(this.client.Ip + "/logs/skipped/");
    ...
    toggleEncoderLog: async function () {
      ...
      try {
        const lineOut = await get(this.client.Ip + "/encoder/");
    ...
    clearClient: function () {
      ...
      this.client.Status = "offline";
      this.client.Paused = false;
      this.client.InFile = "";
      this.client.PauseReason = "";
      this.showMainLog = false;
      this.showErrorLog = false;
      this.showProcessedLog = false;
      this.showSkippedLog = false;
      ...
      try {
        ...
        if (response == "paused") {
          try {
            ...
            const refreshResponse = await get(url);
